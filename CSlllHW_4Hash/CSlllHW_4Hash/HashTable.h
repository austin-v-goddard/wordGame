...
public:
	...
	HashRecord * insert(const HashKey x, HashRecord * h);    // what does this return?
	HashTable(int size = 977) : hashTable(nextPrime(size))   // what does this do?
	{
		makeEmpty();
	};
	HashRecord *find(const HashKey & x) const;     //why const?
	void makeEmpty();
...
string HashTable<HashKey, HashRecord>::toString(int howMany)
{
	...
	ss << "Active Elements " << activeElements << endl << " ";
...
{
	activeElements = 0;
	for (auto & entry : hashTable)  // Thats one wicked for loop.  loops through entire loop.  
		entry.info = EMPTY;
};
